...
def build_prompt(processor: AutoProcessor, image: Image.Image) -> str:
    ...
    return processor.apply_chat_template(message, tokenize=False, add_generation_prompt=True)


def main() -> None:
    parser = argparse.ArgumentParser(description="Inference for Qwen2-VL on STL renders using vLLM")
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default=str(Path(__file__).parent / "work_dirs" / "qwen2vl_sampling_finetune_cadevolve_debug" / "final_model"),
    )
    ...
    def flush_batch() -> None:
        ...
        for request_out, stl_path in zip(outputs, batch_paths):
            if not request_out.outputs:
                ...
                completion_tokens = len(best.token_ids) if best.token_ids is not None else 0
