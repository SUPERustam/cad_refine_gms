...
- Runtime artifacts under `cad_rl.runtime` are the source of truth for run directories, manifests, checkpoint records, and stage handoffs on disk. Prefer extending those records over introducing parallel state-tracking paths.
- Preserve a thin root CLI that resolves config, optionally prints a dry-run contract, and delegates to package entrypoints. Keep the workflow surface as subcommands on `cli.py`: `prepare-dataset`, `train`, `resume-train`, `infer`, `build-meshes`, `evaluate`, `compare-runs`. `resume-train` keeps the user-facing `--checkpoint` selector; if resume behavior changes, update the CLI and runtime contract together.
...
- When touching evaluation/comparison artifacts, prefer explicit run and checkpoint identity in summaries and reports. The current comparison flow can fall back to summary file paths when summaries omit that metadata; avoid deepening that fallback.
