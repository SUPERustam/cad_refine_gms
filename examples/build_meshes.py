...
# Limit BLAS back-end threads (each worker gets exactly one core)
...
def _iou(gt_mesh, pred_mesh):
    try:
        ...
        # print(gt_volume, pred_volume, intersection_volume)
...
def _metrics_for_snippet(code: str, gt_path: str, n_points: int, pred_mesh_path: str):
    """*pred_mesh_path* is the STL path without the ``.stl`` suffix."""
    ...
    try:
        pred_mesh.export(f"{pred_mesh_path}.stl")
        ...
        print(f"[{base_name}] Export/metrics failed: {exc}")
        return dict(file_name=base_name, cd=None, iou=None)
...
def _run_with_timeout(args: Tuple[str, str, int, str], timeout: int = 60):
...
def compute_metrics_for_files(
    ...
    *,
    ...
    Unnormalized prediction STLs go under ``dirname(cad_files[0])/meshes`` (all
    *cad_files* are expected to share the same parent directory).
    """
    # Pool is global so that child processes are auto-joined on interpreter exit
    global _POOL  # pylint: disable=global-statement

    if "_POOL" not in globals():
        ctx = get_context("forkserver")  # each worker is clean Python
    ...
    if cad_files:
        pred_root = os.path.dirname(os.path.abspath(cad_files[0]))
        meshes_dir = os.path.join(pred_root, "meshes")
    ...
    for cad_path in cad_files:
        ...
        stem = os.path.basename(cad_path)
        stem = stem[:-4]
        pred_mesh_path = os.path.join(meshes_dir, f"{stem}_0")
        job_args.append((code_str, gt_path, n_points, pred_mesh_path))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--workers", type=int, default=4, help="Number of workers to use"
    )
    parser.add_argument("--timeout", type=int, default=60, help="Timeout in seconds")

    args = parser.parse_args()
    dataset_paths = {
        "mcb": "/scratch/498rustam/datasets/MCB_A_batch/groudtruth/",
        "fusion360": "/scratch/498rustam/datasets/fusion360_test_mesh",
        "deepcad": "/scratch/498rustam/datasets/deepcad_test_mesh",
        "mcb_1000": "/scratch/498rustam/datasets/mcb_a_batch_groundtruth_1000/",
