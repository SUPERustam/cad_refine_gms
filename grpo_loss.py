...
def adv_select_top_samples(self, inputs, num_generations: int, top_samples: int):
    ...
    B = completion_ids.size(
        0
    )  # G * batch size per device / num_iterations --- 16 * 4 = 64
...
def cppo_compute_loss(
    ...
    logits_to_keep = completion_ids_full.size(
        1
    )  # we only need to compute the logits for the completion tokens

    sel_inputs = self.select_top_samples(
    ...
    completion_ids, completion_mask = (
        sel_inputs["completion_ids"],
        sel_inputs["completion_mask"],
    )
