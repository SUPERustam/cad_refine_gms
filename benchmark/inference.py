...
- Takes a folder with .stl files (recursively).
...
Usage:
  python inference_qwen2vl_from_stls.py \
      --stl_dir /path/to/stls \
      --out_dir ./preds \
      --model_path ./work_dirs/qwen2vl_sampling_finetune_cadevolve_debug/final_model \
...
def extract_assistant_text(decoded: str) -> str:
    ...
    if start_tag in decoded:
        ...
        # strip the optional leading newline after assistant
        if part.startswith("\n"):
            part = part[1:]
        if end_tag in part:
            part = part.split(end_tag, maxsplit=1)[0]
        return part.strip()
    # Fallback: return everything (after last newline)
...
def main():
    parser = argparse.ArgumentParser(description="Inference for Qwen2-VL on STL renders")
    ...
    parser.add_argument("--model_path", type=str, default=str(Path(__file__).parent / "work_dirs" / "qwen2vl_sampling_finetune_cadevolve_debug" / "final_model"))
    ...
    # Keep processor resize consistent with training
    ...
    # Load processor/model (match training flags)
    ...
    def flush_batch():
        ...
        with torch.no_grad():
            ...
            # Decode per item and extract assistant text
        ...
        # reset
        batch_imgs, batch_paths = [], []
    # random_stls = np.random.choice(stls, size = 1000)
    # Iterate STLs, render and batch
    print(len(stls))
    for stl in stls:
        try:
