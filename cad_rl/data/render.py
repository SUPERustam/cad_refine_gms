...
try:  # pragma: no cover - optional dataset-prep dependency
    ...
    _VIS_IMPORT_ERROR: Exception | None = None
...
def _raise_missing_vis_dependency() -> None:
    ...
    raise RuntimeError(message) from _VIS_IMPORT_ERROR
...
if _BasePlotter is not None:
    ...
    class Plotter1_1(_BasePlotter):
        def _get_img(self, mesh_path, cmap, apply_augs=False, color=None):
            ...
            extents = maxs - mins
            scale = 1.0 / max(extents.max(), 1e-12)
            mesh.points = (mesh.points - center) * scale + 0.5
            ...
            success = self.iso_plotter.remove_actor(
