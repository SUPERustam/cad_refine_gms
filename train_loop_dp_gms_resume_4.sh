...
CUDA_VISIBLE_DEVICES=0 trl vllm-serve \
  ...
  >"$VLLM_LOG" 2>&1 &
sleep 80
# `script -c` accepts only one command string; extra quoted words become script(1) options.
# Inside double quotes, backslash-newline is removed; leading spaces on continued lines keep tokens separated.
script --flush -c \
